 ...
 */
public final class TextRow implements Row, Serializable, Comparable<Row> {
    ...
    public TextRow(Schema[] header) {
        this.header = header;
        this.size   = header.length;
        this.values = new String[size];
    }

    public TextRow(Schema[] header, String [] arr) {
        this.header = header;
        this.size = header.length;
        this.values = new String[size];
        this.parse(arr);
    ...
    public final Collection<Object> collect() {
        return IntStream.range(0, size)
                .mapToObj(i -> Parser.parse(header[i].getType(), values[i])).collect(Collectors.toList());
    }
    ...
    public final boolean equals(Object o) {
        ...
        if (o == this)
            return true;
        if (o instanceof TextRow) {
            TextRow other = (TextRow) o;
     ...
     * @return 返回实例指向
     */
    public final TextRow parse(String[] arr, int start, boolean truncate) {
        rangeCheck(start);
        modCount++;
    ...
    public final TextRow parse(String[] arr) {
        return parse(arr, 0, true);
    }

    public final TextRow parse(String line, String regex) {
    ...
    public final Object get(String fieldName) {
        return getValue(fieldName).getValue();
    }
    ...
    public final String getString(int index) {
        rangeCheck(index);
        return StringUtils.vaild(values[index], "");
    }
    ...
    public final Value getValue(int index) {
        rangeCheck(index);
        return Parser.parseValue(header[index], values[index]);
    }
     ...
     * 获取修改次数。
     * <p>
     *     每当调用 <code>parse</code> 方法，该计数会自动增加 1。
     * </p>
     * @return 返回被修改次数。
     */
    ...
    public Map<String, Object> toNamedMap() {
        return IntStream.range(0, size)
                .mapToObj(i -> new Node<>(header[i].getName(), Parser.parse(header[i], values[i])))
    ...
    public Map<Schema, Object> toMap() {
        return IntStream.range(0, size)
                .mapToObj(i -> new Node<>(header[i], Parser.parse(header[i], values[i])))
                .collect(Collectors.toMap(Node::getKey, Node::getValue, (k1, k2) -> k1));
        /*Map<Schema, Object> map = new HashMap<>();
        for (int i = 0; i < size; i++) {
            map.put(header[i], getValue(i).getValue());
    ...
    public Map<Schema, Object> toMap(@NotNull String... fieldNames) {
        ...
        //return IntStream.range(0, size).filter(i -> find(fieldNames[i]) > 0)
        //        .mapToObj(i -> new Node<>(header[i], Parser.parse(header[i], values[i])))
        //        .collect(Collectors.toMap(Node::getKey, Node::getValue, (k1, k2) -> k1));
        Map<Schema, Object> map = new HashMap<>();
    ...
    public final Object setValue(int index, Object value) {
        ...
        if (!Parser.matches(header[index].getType(), value))
            throw new IllegalArgumentException(
                    "The Value (" + value + ") can not match the type of " + header[index] + ".");
        String old =  values[index];
        values[index] = String.valueOf(value);
        return Parser.parse(header[index].getType(), old);
    }
    ...
    public final Row split(int startIndex, int endIndex) {
        ...
        if (startIndex < endIndex)
            throw new IllegalArgumentException(
                    "The parameter startIndex must be bigger than endIndex (" + startIndex+ " - " + endIndex + ")");
        if (startIndex == 0 && endIndex == size) {
            return new TextRow(header.clone(), values.clone());
        } else {
            return new TextRow(
