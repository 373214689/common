...
public final class Column implements Schema, Serializable, Cloneable {
    ...
    public static Column parse(String text) {
        ...
        if (text.toLowerCase().indexOf("PRIMARY") != -1) {
            primary = true;
        }
    ...
    private int mutexGenus = 0;
    /** 同属，假如同一个同属组中某一个元素满足，则表示其他的元素都被满足, 0 表示不作同属判定 */
    private int sameGenus = 0;
    ...
    public Column clone() {
        ...
        column.mutexGenus = mutexGenus;
        column.sameGenus = sameGenus;
        return column;
    }
    ...
    /**
     * 设置列数据类型
     * @param typeName 指定的类型名称，如果不存在则会抛出异常。
     */
